...
- 篇章页：`chapter-core-methodology.html` 用 **M**（模块 M01~M07，共 74 卡，如 `M01.01`），`chapter-01~15` 用 **C**（如 `C10.26`）。
- 核心原理页：`E`（如 `E07.02`）。
...
- `ov-stat-num` 顺序（**11 个**，校验权威三源；第 8 项为「方法论 74」，勿遗漏）：
  1. 总量（345）
  2. P0（90）
  3. P1（195）
  4. P2（60）
...
- 统计块 `stat-number`：深度Q&A / 核心原理 / 场景 各自计数、合计（= 题数 + 方法论卡片，如 345+74=419）、全站 N 题。
...
全站 **345** = 篇章 **220** + 核心原理 **58** + 场景 **67**；优先级 P0=**90** / P1=**195** / P2=**60**；难度 专家级 **44** / 架构级 **160** / 高级开发 **141**。统计口径为「篇章 + 核心原理 + 场景」三页，方法论页（74 张 M 卡）与概览页、安全卡页不计入题目数。任何同步后须满足：优先级三项和=总量、难度三项和=总量、类型三项和=总量。
