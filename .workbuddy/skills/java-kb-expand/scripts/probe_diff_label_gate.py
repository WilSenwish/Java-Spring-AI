...
证明这两条新门禁**不是恒真**：
  ...
  轮 3 注入缺陷 B（非难度占用）  → 期望 1j FAIL（根 index 一个 layer-tag 改回 difficulty-architect）
  轮 4 注入缺陷 C（专篇带属性）  → 期望 1k FAIL（chapter-core-methodology 加回 data-difficulty）
  轮 5 还原 + MD5               → 期望两份文件 MD5 与原始一致且 failed=0

只动两个文件，结束时用内存中的原文还原。项目根由「向上查找同时含 AGENTS.md + index.html
的祖先目录」推导，不写死层级。每个缺陷只注入一次（count=1），避免批量破坏。
"""
import hashlib
import os
import re
...
LAYER_TAG = re.compile(r'<span class="layer-tag">想</span>')
...
def main():
    ...
    if not LAYER_TAG.search(f2):
        print("  !! 未找到注入锚点（根 index 无「想」layer-tag），中止")
        return 1
    open(CH1, "w", encoding="utf-8").write(f1)  # 复位 A，保证缺陷互不叠加
    open(ROOT_IDX, "w", encoding="utf-8").write(
        LAYER_TAG.sub('<span class="difficulty difficulty-architect">想</span>', f2, count=1))
    j, k = run("缺陷B")
    if j < 1 or k != 0:
