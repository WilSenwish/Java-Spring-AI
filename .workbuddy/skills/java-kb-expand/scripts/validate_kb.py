...
EDITORIAL_WORDS = ["再加厚", "补厚", "占位段落", "待补写",
                   # 重构批注（2026-09-18 实测）：内容从设计文档/建议文档搬入时带入的层级迁移标记，
                   # 曾以「（升法）」「已升法」「（原 M01.07）」形式残留于标题与正文共 16 处，
                   # 原清单拦不住 → 补入。
                   "（升道）", "（升法）", "（升术）", "（升器）", "（升势）",
                   "已升道", "已升法", "已升术", "升入法层", "（原 M"]
if not os.path.isfile(COUNTS_JSON):
