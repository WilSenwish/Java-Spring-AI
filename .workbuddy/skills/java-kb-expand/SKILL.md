...
- `scripts/check_list_indent.js` — 列表 marker 缩进**实测**（CDP，零依赖）。判定 `markerSpace < 1.2×li字号 − 0.5px`：`markerSpace` = 列表内容盒左边 − 边界容器内容盒左边（边界优先取卡片类 `.epq-card`/`.qa-card`/`details.map-card`，其次取最近有 padding 的块级祖先，**绝不退到 0**，否则 375px 假阳 / 1280px 假阴）。`node check_list_indent.js`（全站 44 页 × {1280, 375}）、`--only <相对路径>`、`--viewport 375`、`--dump`（打印各分组样本）、`--out r.json`。有嫌疑即 exit 1。
