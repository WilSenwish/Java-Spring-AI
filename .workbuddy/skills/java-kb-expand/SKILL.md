...
- 降级动作（按顺序）：
  ...
  2. 仅基于**标题关键词**给出「宿主篇章初判」候选（如：Nacos/配置中心/微服务 → 第 10 篇；epoll/IO/Reactor/网络 → 第 13 篇；JVM/GC → 第 12 篇；分布式事务/锁/ID → 第 09 篇；缓存/Redis → 第 07/08 篇；MQ → 第 06 篇；数据库/分库分表 → 第 05 篇；Spring/Bean/IoC/AOP → 第 02/03 篇），并**显式标注"未经正文覆盖度核查"**。
...
2. **补层轮次也要做"计数体检"（2026-09-03 新增，高收益）**：即使本轮不改计数，也应顺手全量核对 4 份聚合页，因为历史轮次常留下孤儿数字。已实测抓出 3 处遗留错误，务必覆盖三类隐藏位：
   - **① `stat-number` 全部字段**（不只看"全站 N 题"）。根 index 的「**题目总数**」曾长期停留在 **413**（＝旧合计 339+74 的值被误填），正确值应＝题数 345；而「合计」= 题数 + 方法论卡片 = 419 本身没错。**题目总数 ≠ 合计**，改题数时两个都要动。
...
| `chapter-overview-priority.html` | 新增 ov-item；`ov-stat-num` 共 **11 个**字段须同步 +N：**总量 / P0 / P1 / P2 / 专家级 / 架构级 / 高级开发 / 方法论 / 章节题 / 核心原理 / 场景题**（第 8 项「方法论 74」易漏，务必逐项核对）；对应子组标题（优先级×难度 9 宫格）+N |
...
- **overview `ov-stat-num` 共 11 项全要改**：总量/P0/P1/P2/专家级/架构级/高级开发/方法论/章节题/核心原理/场景题。最易漏的是「章节题」（篇章数）与「方法论」（第 8 项，固定 74），漏改会导致 overview 与根 index、章节 index 的计数不一致。
- **根 index 合计 ≠ 题数**：合计 = 题数 + 方法论卡片数（如 345+74=419），加题时题数与合计各 +1。
