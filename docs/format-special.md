...
- **定位**：核心思维方法论，<span class="kb-count" data-kb-count="methodology" data-kb-pos="P16">94</span> 张卡片，ID 前缀 `M`（如 `M01.01`），按模块 `M01~M16` 共 16 组分层侧边目录。
...
- **难度**：**不适用 —— M 卡不分难度等级**（2026-09-16 长官决策：移除 `data-difficulty`、难度计数键与「方法论难度三级求和」不变式）。本篇合计 <span class="kb-count" data-kb-count="methodology" data-kb-pos="P17">94</span> 卡。
...
- **结构层级**：
  ...
  ├─ （方法论 <span class="kb-count" data-kb-count="methodology" data-kb-pos="P18">94</span> 卡不在本页统计条，只在专篇页与各 index 呈现）
