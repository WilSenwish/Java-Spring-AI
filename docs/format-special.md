...
- **定位**：全库框。C/E/S 是领域细枝，G 是门禁化的器，K 是器失效的反例；本篇提供可挂载的道/法/术/势（器**不单列** Module）。势已独立为 **M16 维度**（势定律 11 卡），表达与对齐另立 **M17（附）**。开篇含五层定义、同一词跨层三例、全组×五层挂网总表与四种错位；**每组必有** `.mod-hang` 五列下挂。<span class="kb-count" data-kb-count="methodology" data-kb-pos="P16">149</span> 张卡片，ID 前缀 `M`（如 `M01.01`），按模块 **`M01~M17` 共 17 组**分层侧边目录。
...
- **难度**：**不适用 —— M 卡不分难度等级**（2026-09-16 长官决策：移除 `data-difficulty`、难度计数键与「方法论难度三级求和」不变式）。本篇合计 <span class="kb-count" data-kb-count="methodology" data-kb-pos="P17">149</span> 卡。
- **优先级**：**属性侧**仅部分卡片携带 `data-priority`（分子/分母以 SSOT 键 `methodology_with_priority` / `methodology` 为准，**勿写死**；展示位在 `format-shared.md` §3.2）；**可见徽标侧不呈现**（同日决策：M/G/K 去等级与优先级徽标）。不属于全站题目优先级口径（P0/P1/P2 只统计 C+E+S = `total`）。
...
- **结构层级**：
  ...
  ├─ （方法论 <span class="kb-count" data-kb-count="methodology" data-kb-pos="P18">149</span> 卡不在本页统计条，只在专篇页与各 index 呈现）
