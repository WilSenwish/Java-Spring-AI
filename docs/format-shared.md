...
| `probe_count_coverage_gate.py` | 计数**覆盖**门禁（`validate_kb` 0c = `audit_l1_counts.py`）正向验证 | **六类**注入：① 去保护「M## 名（N）」② 容器内裸「99 卡」③ **白名单外的全新容器样式内裸计数**（容器枚举必漏 → 证明兜底生效）④ **短块兜底**（layer-count 去保护）⑤ **正文长段落「15 篇」去保护**（短块兜底覆盖不到）⑥ **长正文注入全新裸计数**（400 道 / 149 张）→ 必须 FAIL，还原后 PASS |
| `audit_prose_exempt.py` | **豁免台账**（只读，非 probe）：正文聚合门禁的全部候选及**豁免归属**逐条可见 | 输出「未被豁免」应为 **0 处**；改 `prose.exempt` 后必看 —— **豁免本身是盲区来源，必须可见** |
...
| `probe_truth_gate.py` | **真源**审计（`audit_truth_source.py` = `validate_kb` 0c2 步）正向验证 | **六类**注入：A **值错但 DOM + SSOT 四处自洽**（`sync_counts check` 依然全绿 → **只有真源审计能发现**，本门禁存在的唯一理由）/ B 未声明真源 / C 不变式破损（分项展示值之和 ≠ 合计真值）/ D **键路径冲突**（扁平点分键 + 嵌套键双写）/ E 真源配置写错 / F 声明未生效 → 必须 FAIL，还原后 PASS |
...
- **脚本定位项目根禁止写死层级，更禁止写死绝对路径**：校验/探针脚本若需项目根，须走**全脚本唯一实现**
  `scripts/_kbroot.py::find_root(__file__)`（向上查找同时含 `AGENTS.md` 与 `index.html` 的最近祖先），
  而非 `dirname(__file__)` 叠加固定层数——本技能可经 `.agents/skills/java-kb-expand` 软链调用，层级一变即静默指错目录
  （曾指到 `.workbuddy/` 并报 `FileNotFoundError`，看似「门禁通过」实则未执行到判定）。
  2026-09-21 另清理 **9 个脚本硬编码「某用户家目录 + 项目名」绝对路径**——换机器 / 换目录 / 换用户即全崩，
  固化为「技能目录内脚本一律 `find_root` + SSOT 配置驱动，一次性临时脚本才用绝对路径」。
- **只比结构特征，不比展示文本**：跨文件一致性（如 mind 与章节）只比对 **ID 集合**，标题/措辞错位不报错；
  避免门禁因文案微调而误杀，也避免文案漂移漏报结构问题。
- **门禁配置数据化，脚本内禁止写死编码子集**（2026-09-21 长官指令「极易遗漏」）：
  门禁的**扫描范围 / 容器白名单 / 单位词 / 模式正则 / 兜底参数 / 正文参数 / **真源声明（`guard.sources`）** / 层归属区间 / 派生计数真源 / 白名单 / 忽略目录 / 项目根层级**一律写入
  `docs/kb-counts.json` 的 **`guard` 块**，脚本运行时读取（缺键才回落内置默认）；
  **禁止**在脚本里写死「组号区间」「文件名清单」「容器名列表」「单位词表」「绝对路径」——写死一个子集必然遗漏集合外的一切。
  历史事故：`audit_count_drift.py` 初版写死层归属区间、只审计 3 个文件、白名单硬编码单一位号；
  `run_l1_container_scan` 的容器与单位词写死后，mind 页「M## 名（N）」**括号式组数**（14 处）长期无人管；
  9 个脚本硬编码家目录绝对路径。**新增受保护场景只改 SSOT 配置，不改脚本。**
  配置键清单与消费方见 `conventions.md` §5.1.4「计数保护全覆盖」。
- **计数保护不分「页面装饰」与「正文」**（2026-09-21 长官追加指令「正文聚合计数也要应管尽严」）：
  门禁只覆盖「页头 / footer / 卡片元数据」是不够的 —— **埋在长正文段落里的聚合计数同样是展示计数**。
  计数类范式 = `guard.coverage.prose`（D 段）：判据锚定**语义真值集 V**（= `counts` 全部值 ∪ 已注册
  position 键的当前值），凡正文里「数字 ∈ V 且紧邻单位词」而未被 `data-kb-pos` span 包裹即 FAIL。
  这是**真值锚定而非句式/容器枚举**，故新写法的正文计数自动被覆盖；技术叙述数字天然不命中 V。
  误报一律进 `prose.exempt`（**每条须写理由**），**禁止调 `prose.units` / 关 `prose.enabled`**；
  并用 `scripts/audit_prose_exempt.py` 复核豁免台账（豁免不得成为看不见的盲区）。
  **边界（2026-09-21 收窄）**：D 段只判「有没有受保护」，**不管数写得对不对**；但**已登记且已声明真源的计数位，
  其值的正确性由 0c2 步（`audit_truth_source`）负责**（见下「登记了位置点就必须有正确性校验」）。
  残余边界仅剩：**正文里那些不对应任何 SSOT 键的自由数字**（如误写「163 张卡」）——无键可锚，属人审范畴。
  勿据此认为门禁失效：只要它是个计数位，就必然落在某条真源规则下。
...
- **登记了位置点，位上的数就必须有正确性校验**（2026-09-21 长官指令「不然门禁有啥意义」）：
  前三类门禁（覆盖 / 一致 / 结构）**全部是内部自洽** —— `position ↔ SSOT ↔ DOM` 三方对齐，
  **没有一条链路核对「那个值本身对不对」**。实测后果：根 index 组 1 显示 16（实际 17 项）、
  6 篇章节索引页「高级」少 1–3、核心原理「高级」39（实际 40）、SSOT 里同一逻辑键**扁平 + 嵌套双写不同值** ——
  **旧门禁全绿**。故新增 **0c2 步 `audit_truth_source.py`**：按 SSOT `guard.sources` 的
  `rules`（`dom_scan` / `cards_by_link` / `sum` / `file_count` / `page_diff` / `layer_sum` / `derived` / …）与
  `manual` 逐键**独立求真值**，再与 SSOT 键值、DOM 显示值**三方比对**，并校验 `invariants`（分项**展示值**之和 = 合计**真值**）。
  五类 FAIL 全部点名、**默认不截断**：`[未声明真源]` / `[声明未生效]` / `[真值不符]` / `[键路径冲突]` / `[无展示位]`。
  **新计数注册 position 后必须同步在 `guard.sources` 声明真源**，否则 0c2 直接 FAIL ——
  这与「先加 `data-kb-pos`」是**同级硬要求**，不是可选项。
  **真源不是卡片数的**（如某层下挂元素个数）走 `guard.derived.items`（`region` + `count`，真源 = DOM 元素数，
  且**声明了却没有已注册 position 即 FAIL**，防「有真源无展示位」的空转声明）。
  **禁止绕开**：不得把 `guard.sources` 的口径改成「真值 = 当前值」来换绿灯；发现 `[真值不符]` 后**不得只把 SSOT 与 DOM 互相对齐**
  （两边一起错还是错）——必须先读原文 / 数元素查清**内容真相**，再走 `sync_counts.py bump` 同时改 DOM 与 SSOT。
  边界：真源引擎只覆盖**已被声明的键**；正文里不对应任何键的自由数字仍属人审范畴（同 D 段边界收窄说明）。
- **门禁改动须双向验证并留 probe**：「注入 → 必须 FAIL → 还原并核一致」之外，**新增 / 修改门禁实现或 `guard` 配置后**
  ...
  **`scripts/probe_truth_gate.py`**），证明「注入必 FAIL、还原必 PASS」，否则视为**假绿**。
- **验证脚本自身的纪律（2026-09-21 事故后固化）**：
  - **还原一律 `try/finally`**：某 probe 因「注入锚点找不到」直接 `return 1`，绕过还原，
    把 `chapter-01` 一枚徽标留成「高级开发」，随后 `validate_kb` FAIL —— **会污染被测对象的验证脚本，结论不可信**。
  - **注入锚点动态取**（用当前文件里真实存在的结构定位），**不写死文本、不写死绝对路径、不依赖 `tmp/` 历史夹具**
    （夹具随沙箱清理即消失，脚本抛 `FileNotFoundError`，被误读成「门禁失效」）。
  - 退出码约定：`0` 通过 / `1` 门禁失效 / `2` 定位失败 / **`3` 夹具缺失待重建**（**不得抛异常、不得静默退出 0**）。
